...
soc_analyst = AssistantAgent(
    description=("You are an advanced senior SOC analyst Agent. "
                ...
                "- Use xsoar_info agent to retrieve the incident from XSOAR using the Sentinel incident number (not the ID which is a GUID)."
                "- Once you retreive all data rom XSOAR and Sentinel, use kql_sentinel_query agent to create and execute KQL queries in Microsoft Sentinel Log Analytics workspace.\n"
    ...
    system_message=(
        ...
        "Always get the incident from XSOAR using the sentinel incident number after getting the data from Sentinel itself."
...
kql_sentinel_query = AssistantAgent(
    ...
    system_message=(
        ...
        "Limit the timespan of the queries to a timespan of maximum 3 days around the time of the incident."
        "Do not define a close reason or close notes. That is the responsibility of the soc_analyst agent."
